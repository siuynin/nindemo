...
if ($argc < 2) {
    echo "Usage: php import_pricing_plans_corrected.php <json_filename>\n";
    echo "Example: php import_pricing_plans_corrected.php models_export_2025-09-23_08-14-14.json\n";
    exit(1);
}
...
try {
    ...
    echo "\n=== IMPORTING PRICING PLANS (CORRECTED) ===\n";
    
    // Import PricingPlan data with proper field mapping
    if (isset($data['pricing_plans'])) {
        ...
        foreach ($data['pricing_plans'] as $item) {
            try {
                // Map fields correctly based on table structure
                $planData = [
                    'id' => $item['id'],
                    'name' => $item['name'],
                    ...
                    'features' => isset($item['features']) ? (is_array($item['features']) ? json_encode($item['features']) : $item['features']) : '[]',
